...
class Stock:
    ...
    def update_price(self):
        if self.current_day < len(self.close_prices):
            ...
            self.price = random.choice([self.high_prices[self.current_day], self.low_prices[self.current_day], self.close_prices[self.current_day]])
...
wb = openpyxl.load_workbook("C:\\Users\\kingt\\OneDrive\\Desktop\\New folder\\NVDA.xlsx")
ws = wb.active
...
def get_numerical_data(cell_value):
    if isinstance(cell_value, (int, float)):
        ...
        #Gives error if it cannnot find a numeric value
        except ValueError:
...
high_prices_a = [get_numerical_data(ws.cell(row=i, column=2).value) for i in range(1, 253)]
low_prices_a = [get_numerical_data(ws.cell(row=i, column=3).value) for i in range(1, 253)]
close_prices_a = [get_numerical_data(ws.cell(row=i, column=1).value) for i in range(1, 253)]

high_prices_b = [105, 106, 107, 108, 109, 110, 109, 108, 107, 106, 105, 104, 103, 102, 101, 100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 87, 86]
low_prices_b = [95, 96, 97, 98, 99, 100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 87, 86, 85, 84, 83, 82, 81, 80, 79, 78, 77, 76]
close_prices_b = [100, 101, 102, 103, 104, 105, 104, 103, 102, 101, 100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 87, 86, 85, 84, 83, 82, 81]

high_prices_c = [205, 207, 209, 208, 210, 212, 214, 213, 212, 211, 210, 209, 208, 207, 206, 205, 204, 203, 202, 201, 200, 199, 198, 197, 196, 195, 194, 193, 192, 191]
low_prices_c = [195, 197, 199, 198, 200, 202, 204, 203, 202, 201, 200, 199, 198, 197, 196, 195, 194, 193, 192, 191, 190, 189, 188, 187, 186, 185, 184, 183, 182, 181]
close_prices_c = [200, 202, 204, 203, 205, 207, 209, 208, 207, 206, 205, 204, 203, 202, 201, 200, 199, 198, 197, 196, 195, 194, 193, 192, 191, 190, 189, 188, 187, 186]
